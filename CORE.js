...
mu.on('ready', () => {
  mu.guilds.array().forEach((eachGuild) => {
...
mu.on('message', (msg) => {
  ...
  if ((search === '그만불러' || search === '닥쳐') && msg.content.startsWith(process.env.mupf) && song[msg.channel.id]) {
    ...
    msg.channel.send('쳇... ')
  } else if ((search === '잠만뭠춰봐' || search === '잠만') && msg.content.startsWith(process.env.mupf) && song[msg.channel.id]) {
    song[msg.channel.id].pause()
    msg.channel.send('뮤? 일단 뭠췄다뮤! 다시 키려면 `mz!다시불러`, 다른 노래로 바꾸고 싶다면 `mz!그만불러`를 사용하라뮤!')
  } else if ((search === '계속불러' || search === '플레이') && msg.content.startsWith(process.env.mupf) && song[msg.channel.id]) {
    song[msg.channel.id].resume()
    msg.channel.send('뮤봇이 계속 불러준다뮤~')
